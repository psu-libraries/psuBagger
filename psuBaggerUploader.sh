...
echo "$(date): $bagid.tar has been created and will now be uploaded to APTrust. This may take a while too." 2>&1 | tee -a $log

# Send the bag to APTrust
$local/tools/aptrustPartnerTools/apt_upload $bagid.tar  2>&1 | tee -a $log && echo "$bagid.tar has been received by APTrust." 2>&1 | tee -a $log && echo "$bagid" >> $local/logs/sentBags.txt
